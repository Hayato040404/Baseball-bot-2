...
export async function runRefreshCycle(options?: { allowPush?: boolean }) {
  if (!inScrapeWindowJst()) {
    return { ok: true as const, skipped: true as const, reason: 'outside_scrape_window' as const };
  }
  ...
  const shouldPush = options?.allowPush !== false && didScoreChange;
  const pushResult = shouldPush ? await sendPushToAll(createScorePush(feed.snapshot, newestPlay)) : { sent: 0, removed: 0 };
